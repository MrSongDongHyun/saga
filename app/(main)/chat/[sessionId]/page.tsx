...
type SessionData = { id: string; character: CharacterInfo; title: string | null; messageCount: number; createdAt: string; updatedAt: string };
...
type SettingsPanelProps = {
  ...
  userNote: string; onUserNoteChange: (v: string) => void;
  characterName: string; onOpenImageGen: () => void;
};

function SettingsPanel({ open, onClose, userNote, onUserNoteChange, characterName, onOpenImageGen }: SettingsPanelProps) {
  return (
    <>
      {open && <div className="fixed inset-0 bg-black/40 z-20" onClick={onClose} aria-hidden="true" />}
      <aside className={["fixed top-0 right-0 h-full w-72 bg-bg2 border-l border-bg3 z-30 flex flex-col shadow-xl transition-transform duration-200", open ? "translate-x-0" : "translate-x-full"].join(" ")} aria-label="채팅 설정">
        <div className="h-14 flex items-center justify-between px-4 border-b border-bg3 shrink-0">
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center text-t2 hover:text-t1 transition-colors rounded-lg hover:bg-bg3" aria-label="닫기">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
          <section className="flex flex-col gap-2">
            ...
            <p className="text-xs text-t2 leading-relaxed">{characterName}과의 대화에서 AI가 항상 기억했으면 하는 정보를 입력하세요.</p>
            <textarea
              value={userNote} onChange={(e) => onUserNoteChange(e.target.value)}
          ...
          <section className="flex flex-col gap-2">
            ...
            <button type="button" onClick={onOpenImageGen} className="w-full border border-bg3 hover:border-red/40 text-t2 hover:text-t1 text-xs py-2 rounded-lg transition-colors">이미지 생성 열기</button>
          </section>
        </div>
      </aside>
    </>
...
export default function ChatPage() {
    ...
    async function loadSession() {
      try {
        ...
        if (!sessionRes.ok) {
          ...
          const msg = typeof data === "object" && data !== null && "error" in data && typeof (data as { error: unknown }).error === "string" ? (data as { error: string }).error : "세션을 불러올 수 없습니다.";
  ...
  const handleSend = useCallback(async (content: string) => {
    if (isSending) return;
    const tempUserMessage: ChatMessage = { id: `temp-${Date.now()}`, role: "USER", content, createdAt: new Date().toISOString() };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("[ChatPage] 메시지 전송 실패:", res.status); return;
      }
      const data: SendMessageResponse = await res.json();
      setMessages((prev) => [...prev.filter((m) => m.id !== tempUserMessage.id), data.userMessage, data.assistantMessage]);
    } catch (err) {
      console.error("[ChatPage] 메시지 전송 오류:", err);
      setMessages((prev) => prev.filter((m) => m.id !== tempUserMessage.id));
    } finally {
      setIsSending(false);
      setIsTyping(false);
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-bg z-10">
      ...
      <header className="h-14 bg-bg2 border-b border-bg3 flex items-center px-4 gap-3 shrink-0">
        <button type="button" onClick={() => router.back()} aria-label="뒤로 가기" className="w-8 h-8 flex items-center justify-center text-t2 hover:text-t1 transition-colors rounded-lg hover:bg-bg3">
        ...
        {character.avatar ? (
          ...
          <div className="w-8 h-8 rounded-full bg-red/20 text-red flex items-center justify-center text-xs font-bold shrink-0 select-none">{character.name[0]?.toUpperCase() ?? "?"}</div>
        )}
        <span className="text-t1 font-semibold text-sm truncate flex-1">{character.name}</span>
        {userNote.trim().length > 0 && <span className="text-xs text-red/70 shrink-0">메모 ON</span>}
        <button type="button" onClick={() => setSettingsOpen(true)} aria-label="설정" className="w-8 h-8 flex items-center justify-center text-t2 hover:text-t1 transition-colors rounded-lg hover:bg-bg3">
      ...
      <SettingsPanel
        ...
        userNote={userNote} onUserNoteChange={handleUserNoteChange}
      ...
      {/* 이미지 생성 모달 */}
      <ImageGenModal
        open={imageGenOpen} onClose={() => setImageGenOpen(false)}
        ...
        {messages.length === 0 && !isTyping && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-8">
            {character.avatar ? (
              ...
              <div className="w-16 h-16 rounded-full bg-red/20 text-red flex items-center justify-center text-2xl font-bold select-none">{character.name[0]?.toUpperCase() ?? "?"}</div>
            )}
            <p className="text-t1 font-semibold">{character.name}</p>
            {character.description && <p className="text-t2 text-sm leading-relaxed max-w-xs">{character.description}</p>}
            {character.firstMessage && (
              <div className="mt-2 bg-bg2 rounded-2xl rounded-tl-sm px-4 py-3 text-sm text-t1 max-w-xs text-left">{character.firstMessage}</div>
            )}
          </div>
        )}
        {messages.map((msg) => (
          <MessageBubble key={msg.id} role={msg.role} content={msg.content} createdAt={msg.createdAt} characterName={character.name} characterAvatar={character.avatar} />
        ))}
